...
public class PinYin
{
    ...
    {
        var rs = Get(ch);
        if (!rs.IsNullOrEmpty()) return rs[0];
    ...
    public static String Get(Char ch)
    {
        ...
        {
            var pos = Array.IndexOf(_py3, ch);
            if (pos >= 0) return _pyValue3[pos];
        ...
        {
            var pos = Array.IndexOf(_py2, ch);
            if (pos >= 0)
            {
                var py = _pyValue2[pos].ToArray();
                py[0] = Char.ToUpper(py[0]);
                return new String(py);
            }

            pos = Array.IndexOf(_py3, ch);
            if (pos >= 0) return _pyValue3[pos];
        }
        #endregion 中文字符处理
    ...
    /// <param name="str">汉字字符串</param>
    /// <returns>转换后的拼音(全拼)字符串</returns>
    public static String Get(String str) => GetAll(str).Join("");
}