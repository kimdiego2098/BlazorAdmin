...
public static class JSRuntimeExtensions
{
    ...
    {
        return await jsRuntime.InvokeAsync<Rectangle>("BlazorDiagrams.getBoundingClientRect", element);
    }
        ...
        DotNetObjectReference<T> reference) where T : class
    {
        try
        {
            await jsRuntime.InvokeVoidAsync("BlazorDiagrams.observe", element, reference, element.Id);
        }
    ...
    {
        await jsRuntime.InvokeVoidAsync("BlazorDiagrams.unobserve", element, element.Id);
    }
}
