...
public partial class SysRolePage
{
    ...
    {
        var data = (await SysRoleService.OwnUserAsync(id)).ToList();
        ...
        var option = new DialogOption()
        {
            ...
            BodyTemplate = BootstrapDynamicComponent.CreateComponent<UserChoiceDialog>(new Dictionary<string, object?>
            {
                ...
                [nameof(UserChoiceDialog.ValuesChanged)] = (List<long> v) => OnGrantUserValueChanged(v, id)
            }).Render(),
    ...
    private List<long> GrantUserChoiceValues = new();
    private async Task OnGrantUserValueChanged(List<long> values, long roleId, bool change = false)
    {
