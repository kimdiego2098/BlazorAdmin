...
public class BlazorAppContext
{
    ...
    public async Task InitMenus(string url)
    {
        if (UserManager.UserId > 0)
        {
            ...
            if (module != null && !AllMenus.Any(a => a.Href == url && a.Module == CurrentUser.DefaultModule))
                CurrentModuleId = module.Value;
            else
                CurrentModuleId = CurrentUser.DefaultModule;

            if (CurrentModuleId == 0 || AllResource.FirstOrDefault(a => a.Id == CurrentModuleId && a.Category == ResourceCategoryEnum.Module) == null)
            {
