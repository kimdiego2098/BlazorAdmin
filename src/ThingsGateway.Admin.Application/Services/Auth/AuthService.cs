...
public class AuthService : IAuthService
{
    ...
    private async Task<LoginOutput> ExecLogin(LoginPolicy loginPolicy, LoginInput input, SysUser sysUser, bool isCookie = true)
    {
        ...
        return new LoginOutput
        {
            ...
            Id = sysUser.Id,
            Razor = (await _sysResourceService.GetMenuByMenuIdsAsync(new List<long>() { (await _userCenterService.GetLoginWorkbenchAsync(sysUser.Id).ConfigureAwait(false)).Razor }).ConfigureAwait(false)).FirstOrDefault()?.Href ?? "/",
