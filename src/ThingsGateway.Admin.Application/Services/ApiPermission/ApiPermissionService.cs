...
internal sealed class ApiPermissionService : IApiPermissionService
{
    ...
    public ApiPermissionService(IApiDescriptionGroupCollectionProvider apiDescriptionGroupCollectionProvider
    ...
    /// <inheritdoc />
    public async Task<List<OpenApiPermissionTreeSelector>> ApiPermissionTreeSelectorAsync()
    {
        var cacheKey = $"{nameof(ApiPermissionTreeSelectorAsync)}-{CultureInfo.CurrentUICulture.Name}";
        ...
        if (permissions == null)
        {
            ...
            foreach (var apiDescriptionGroup in apiDescriptions)
            {
                ...
                OpenApiPermissionTreeSelector openApiPermissionTreeSelector = new() { ApiName = apiDescriptionGroup.GroupName ?? "Default" };
