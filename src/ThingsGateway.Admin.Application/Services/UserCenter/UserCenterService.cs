...
internal class UserCenterService : BaseService<SysUser>, IUserCenterService
{
    ...
    public async Task<WorkbenchInfo> GetLoginWorkbenchAsync(long userId)
    {
        ...
        }
        {
            ...
            {
                //如果有数据直接返回个人主页
                relationUserWorkBench.Razor = sysRelation.ExtJson!.FromJsonNetString<long>();
            }
            else
            {
                //如果没数据去系统配置里取默认的主页
                var devConfig = appConfig ??= await _configService.GetAppConfigAsync().ConfigureAwait(false);
                relationUserWorkBench.Razor = devConfig.PagePolicy.Razor;//返回主页信息
            }
        }
    ...
    public async Task UpdateWorkbenchInfoAsync(WorkbenchInfo input)
    {
        ...
        await _relationService.SaveRelationAsync(RelationCategoryEnum.UserDefaultRazor, input.Id, null, input.Razor.ToJsonNetString(),
            true).ConfigureAwait(false);
    }
