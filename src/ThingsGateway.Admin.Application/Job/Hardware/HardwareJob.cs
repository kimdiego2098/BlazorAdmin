        ...
        if (HardwareInfoOptions.Enable)
        {
            try
            {
                if (HardwareInfo.MachineInfo == null)
                {
                    ...
                    if (HardwareInfoOptions.HistoryInterval < 120) HardwareInfoOptions.HistoryInterval = 120;
                    ...
                    HardwareInfo.UUID = DESCEncryption.Encrypt(HardwareInfo.MachineInfo.UUID + HardwareInfo.MachineInfo.Guid + HardwareInfo.MachineInfo.DiskID);
            ...
            try
            {
                if (HardwareInfoOptions.Enable)
                {
                    if (DateTime.Now > hisInsertTime.Add(TimeSpan.FromSeconds(HardwareInfoOptions.HistoryInterval)))
                    {
